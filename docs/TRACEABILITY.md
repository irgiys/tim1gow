...
|---|---|---|---|---|
| BR-01 | Plafon di luar Rp 5 juta – Rp 500 juta ditolak saat submit | `backend/internal/service/pengajuan_service.go`; batas dibaca dari `parameter_umum` (`batas_plafon_min`/`batas_plafon_maks`), di-seed `migrations/000006_seed_batas_plafon` | `internal/service/pengajuan_service_test.go` (`TestBuatPengajuan_BR01_BatasPlafon` — 4.999.999 ditolak & 5jt/500jt diterima, `TestBuatPengajuan_BR01_BatasDibacaDariParameter`, `TestBuatPengajuan_BatasBelumDiaturDitolak`), `internal/httpapi/pengajuan_http_test.go` (`TestRoute_FR02_PlafonDiLuarBatasDitolak422`) | **Done** |
...
| BR-12 | Nomor referensi `IMT-YYYYMMDD-NNNN` unik, tidak dipakai ulang | `backend/internal/service/pengajuan_service.go` (dibangkitkan di server) + constraint `UNIQUE` (`migrations/000003…up.sql:6`) | `internal/service/pengajuan_service_test.go` (`TestFormatNomorReferensi`, `TestBuatPengajuan_BR12_NomorReferensiTidakPernahDiulang`, `TestBuatPengajuan_AC01_NomorReferensiDanStatusDraft`), `internal/httpapi/pengajuan_http_test.go` (`TestRoute_FR02_PengajuanTerdaftarDanNomorReferensiDariServer`) | **Done** — repository nyata (sequence per tanggal) belum ada; saat ini kontraknya diuji lewat fake |
