...
| FR-06 | Skoring Kelayakan Mikro | P0 | AC-06, AC-07, AC-08 | *(belum; service siap, handler milik FR-06 UI)* | `backend/internal/service/skoring_service_test.go` | #4 | In Progress |
| FR-07 | Perhitungan Margin / Nisbah | P0 | AC-09 | *(belum; service siap)* | `backend/internal/service/margin_service_test.go` | #4 | In Progress |
| FR-08 | Approval Berjenjang | P0 | AC-10, AC-11 | `POST /api/pengajuan/{id}/ajukan-approval`, `POST /api/pengajuan/{id}/approval`, `GET /api/pengajuan/{id}/approval` | `internal/service/approval_service_test.go`, `internal/httpapi/approval_http_test.go` | #8 | Done |
| FR-09 | Audit Trail | P0 | AC-08, AC-12, AC-13 | `GET /api/pengajuan/{id}/audit`, `GET /api/audit` *(append-only, tanpa PUT/PATCH/DELETE)* | `internal/service/audit_service_test.go`, `internal/httpapi/audit_http_test.go` | #8 | Done |
| FR-10 | Pembiayaan Kelompok (Majelis) | P1 | AC-14 |  |  |  |  |
| FR-11 | Notifikasi Perubahan Status | P1 | — |  |  |  |  |
| FR-12 | Dashboard Pipeline | P1 | — |  |  |  |  |
| FR-13 | Parameter Terkonfigurasi | P1 | AC-15 | *(CRUD ADM belum; tabel & migrasi siap)* | `skoring_service_test.go` (`TestHitung_AC15_UbahBobotLangsungBerlaku`), `margin_service_test.go` (`TestValidasi_AC15_UbahRentangLangsungBerlaku`) | #4 | In Progress |
